...
class ExcelHelper(object):
    ...
    def find_sheet(self, number):
        """激活操作的sheet"""
        try:
            self.sheet = self.myexcel.Worksheets(number).Activate()
    ...
    def get_col_values(self, col_num):
        """获取列数据"""
        col = self.excelApp.Columns(col_num).Value
        return col

    def get_cell_value(self, row, col):
        """获取某个单元格的值（传入具体行列位置）"""
        return self.excelApp.Cells(row, col).Value
    ...
    def set_x_valuerange_spacing(self, pic_name, x_value_range_start, x_value_range_end):
        ...
        self.myexcel.ActiveChart.FullSeriesCollection(1).Values = "=Sheet2!$K${0}:$K${1}".format(
            str(x_value_range_start),
            str(x_value_range_end))
        self.myexcel.ActiveChart.FullSeriesCollection(2).Values = "=Sheet2!$L${0}:$L${1}".format(
            str(x_value_range_start),
            str(x_value_range_end))
        self.myexcel.ActiveChart.FullSeriesCollection(2).XValues = "=Sheet2!$J${0}:$J${1}".format(
            str(x_value_range_start),
            str(x_value_range_end))
        # X 轴间隔
